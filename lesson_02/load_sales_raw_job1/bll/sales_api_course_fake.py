...
from support_tools.basic_validators import BasicDateValidator, RelativeFilePathValidator
...
class SalesApiBllCourseFake(SalesApiBlInterface):
    ...
    def __init__(self, sales_api_dal: SalesDalInterface, storage_api_dal: StorageDalInterface,
        ...
        self.__path_validator = RelativeFilePathValidator()
    ...
    def save_sales_to_local_disk(self, log: LogItemInterface, date_str: str, raw_dir: str) -> None:
        ...
        if not self.__path_validator.validate(raw_dir):
            raise SalesBllPathFormatException(
                f"Path format is wrong. Must be {self.__path_validator.format}'. raw_dir='{raw_dir}")
        ...
        while True:
            try:
                ...
                norm_raw_dir = os.path.normpath(os.path.normcase(raw_dir))
                logical_file_path = os.path.join(norm_raw_dir, file_name)
                self.__storage_api_dal.save(log, sales_list, logical_file_path)
